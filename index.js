...
monitor.on('up', (res) => logger(`${res.address}:${res.port} IS ONLINE!!`, "[ MR-RAHAD ]"));
...
app.listen(port, () =>
	logger(`Your app is listening a http://localhost:${port}`, "[ MR-Rahad ]")
     );      


logger("Opened server site...", "[ MR-RAHAD ]");
logger("Opening Uptime Monitor by       MR-Rahad...", "[ Starting ]")
...
function startBot(message) {
    (message) ? logger(message, "[ MR-Rahad ]") : "";

    const child = spawn("node", ["--trace-warnings", "--async-stack-traces", "Rahad.js"], {
        cwd: __dirname,
        stdio: "inherit",
        shell: true
    });
  ...
  child.on("error", function(error) {
    logger("An error occurred: " + JSON.stringify(error), "[ MR-Rahad ]");
  });
...
axios.get("https://raw.githubusercontent.com/MR-RAHAD-511/Rahad-bypass2/main/package.json").then((res) => {
  logger(res['data']['name'], "[ MR-Rahad ]");
  logger("Version: " + res['data']['version'], "[ MR-Rahad ]");
  logger(res['data']['description'], "[ MR-Rahad ]");
});
startBot();
